...
function Module() {
  ...
  // killall -9 php-fpm && SERVEPHP_WEBROOT='/var/www' DEBUG=serve-php PORT=8095  node examples/middleware.js
  
  // Define initial settings.
  self.set({
    cwd: process.cwd(),
    pid: process.pid,
    binPath: process.env.SERVEPHP_BINPATH || '/usr/sbin/php-fpm', // @todo Should use which() to identify.
    webRoot: process.env.SERVEPHP_WEBROOT || path.resolve( "./static/fixtures/www" ),
    configPath: process.env.SERVEPHP_CONFIGPATH || path.resolve( './static/etc/php-fpm.conf' ),
    iniPath: process.env.SERVEPHP_INIPATH || path.resolve( './static/etc/php.ini' )
  })
  ...
  async.auto({
    ...
    getMounted: function getMounted( done, report ) {
      Module.debug( 'getMounted. webRoot: %s', self.get( 'webRoot' ) );
    ...
    prepareEnvironment: function prepareEnvironment( done, report ) {
      ...
      temp.mkdir( 'serve-php', function haveWorkingDirectory( error, workingDirectory ) {
        ...
        self.set({
          ...
          //socketPath: process.env.SERVEPHP_SOCKETPATH || path.join(workingDirectory, 'php5-fpm.sock' )        
        ...
        self.get( 'parent' ).use (require('response-time' )( 5 ) );
    ...
    spawnPHP: [ 'getMounted', 'prepareEnvironment', function spawnPHP( done, report ) {
      ...
      self._php.stdout.on( 'data', function(data) {
        Module.debug( 'php-fpm: %s', data.toString() ); 
      });
      
      self._php.stderr.on( 'data', function(data) {
        Module.debug( 'php-fpm: %s', data.toString() ); 
      });
      
      self.set({
        'fpmPID': self._php.pid
      })
  
      // @temp This simulates, in practice we don't want to trigger ready until we're sure that php-fpm has started without issues.
      setTimeout(function() {
